...
import { 
    ...
    Phone, FileText, MapPin, Calendar, Clock, Menu, X, ArrowLeft
} from "lucide-react";
...
const ManageRealtors = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            // 2. Fetch realtors in these cities
            const { data, error } = await supabase
                .from("real_estate_realtors" as any)
                .select("*")
                .in("city", cityNames)
        ...
        <div className="min-h-screen bg-background p-4 lg:p-8">
            <div className="max-w-7xl mx-auto space-y-8">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-black">Gestão Regional de Corretores</h1>
                        <p className="text-muted-foreground">Aprovação e monitoramento nas cidades: {managedCities.join(", ")}</p>
                    </div>
                </div>
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="bg-muted/30 border-b border-muted/50">
                                        ...
                                        <th className="p-4 text-[10px] font-black uppercase tracking-widest text-muted-foreground">Cidade</th>
                                ...
                                <tbody className="divide-y divide-muted/30">
                                    {filteredRealtors.map((realtor) => (
                                        <tr key={realtor.id} className="hover:bg-muted/10 transition-colors group">
                                            ...
                                            <td className="p-4 text-sm font-medium text-muted-foreground">{realtor.city}</td>
                ...
                <DialogContent className="max-w-md rounded-[2.5rem] p-0 overflow-hidden border-none shadow-2xl">
                    ...
                    <div className="p-8 space-y-6 bg-white">
                        ...
                        <div className="space-y-1">
                            ...
                            <p className="font-bold text-gray-800">{selectedRealtor?.city}, {selectedRealtor?.state}</p>
                        </div>
                    </div>
